...
var OrbitControls = function(object, domElement) {
  ...
  //--------------------------------------------------------
  this.updateTween = time => this.targetTween && TWEEN.update(time);
  this.moveTo = (destinationVector, time = 1000) => {
    if (!destinationVector instanceof Vector3) return;
    const destination = destinationVector.clone();
    if (this.targetTween) this.targetTween.stop();
    // const vec = new Vector3(0, 0, 0);
    const current = this.target.clone();
    this.targetTween = new TWEEN.Tween(current)
      .to({ x: destination.x, y: destination.y, z: destination.z }, time)
      // .to( { x: destination.x, y: destination.y, z: destinationVector.z }, 5000 )
      .easing(TWEEN.Easing.Cubic.InOut)
      .onUpdate(() => {
        // vec.set(current.x, current.y, current.z);
        this.goTo(current);
  ...
  this.rotateTo = (left, up, radius, time = 1000) => {
    ...
    this.rotationTween = new TWEEN.Tween(from)
      ...
      .onUpdate(function() {
        ...
        if (Number.isFinite(radius)) {
          const offset = scope.object.position
            .clone()
            .sub(scope.target)
            .length();
  ...
  this.cameraTo = (target, theta, phi, radius, time = 1000) => {
    this.rotateTo(theta, phi, radius, time);
    this.moveTo(target, time);
  };
